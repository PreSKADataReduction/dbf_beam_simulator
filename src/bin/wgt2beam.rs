...
fn main(){
    ...
    let wgt=read_img::<f64>(matches.value_of("wgt").unwrap().to_string(), 0).unwrap().into_dimensionality::<Ix2>().unwrap();
    let h=wgt.shape()[0];
    let w=wgt.shape()[1];
    ...
    let array_beam=quarter_wgt2pattern(
        wgt.slice(s![h/2..h, w/2..w]).view()
        , d, freq_mhz, nside);

    let mut total_beam:Vec<_>=array_beam.iter().zip(ant_beam.iter()).map(|(&a,&b)| a*b).collect();
    
    let beam_norm={
        let (p, w)=get_interpol_ring(nside, SphCoord::<f64>::new(0.0, 0.0));
        p.iter().zip(w.iter()).map(|(&ipix, &w1)| total_beam[ipix]*w1).sum::<f64>()
    };
    ...
    write_map(matches.value_of("out_beam").unwrap(), &[&total_beam], false, true);
}
