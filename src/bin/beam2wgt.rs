...
fn main(){
    ...
    let array_size=matches.value_of("array_size").unwrap().parse::<isize>().unwrap();
    let d=matches.value_of("spacing").unwrap().parse::<f64>().unwrap();

    let freq_mhz=matches.value_of("freq_MHz").unwrap().parse::<f64>().unwrap();
    
    let mut array_beam:Vec<_>=ant_beam.iter().zip(target_beam.iter()).map(|(&a,&t)|{
        t/a
    }).collect();

    let beam_norm={
        let (p, w)=get_interpol_ring(nside, SphCoord::<f64>::new(0.0, 0.0));
        p.iter().zip(w.iter()).map(|(&ipix, &w1)| array_beam[ipix]*w1).sum::<f64>()
    };
