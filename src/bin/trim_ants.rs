...
pub fn sort_ants_by_grade(ants: &[(isize, isize)]) -> Vec<((isize, isize))> {
    ...
    let mut result=ants.iter().zip(result.iter()).map(|(&a, &w)| (a, w/2)).collect::<Vec<_>>();
...
fn main() {
    ...
    let array_size=matches.value_of("array_size").unwrap().parse::<usize>().unwrap();

    let ants = get_ants(array_size);
    println!("{:?}", ants);
    cfg.insert("full_array", ants.clone());
    
    
    let bl = get_baseline(&ants);
    let bl=bl.iter().map(|(&k, &_v)| k).collect();
    cfg.insert("full_bl", bl);

    let trimed=trim_ants(&ants);
    cfg.insert("trimed_array", trimed.clone());

    let trimed_bl=get_baseline(&trimed);
    let trimed_bl=trimed_bl.iter().map(|(&k, &_v)| k).collect();
    cfg.insert("trimed_bl", trimed_bl);
